...
export default class MongoHelper {
    ...
    static async dbCheck() {
      if (!Object.prototype.hasOwnProperty.call(config, 'connection_string') || config.connection_string === '') {
        console.log('ERROR:: No Connection String specified! Please update the correct connection string in config.js and restart the app');
        process.exit(1);
      }
      if (!Object.prototype.hasOwnProperty.call(config, 'db_name') || config.db_name === '') {
        console.log('ERROR:: No Database Specified. Please update the correct database name in config.js and restart the app');
        process.exit(1);
      }
